    ...
    public class ChatMessage
    {
        ...
        internal IList<ContentItem> ContentItems
        {
            get
            {
                ...
                bool multiModal = Images.Count > 0;
                if (!string.IsNullOrEmpty(TextContent))
                {
                    if (multiModal)
                    {
                        items.Add(new ContentItem(TextContent) { MultiModalFormat = true });
                    }
                    else
                    {
                ...
                if (Images != null && Images.Count > 0)
                {
                    ...
                    {
                        items.Add(new ContentItem(image) { MultiModalFormat = true });
                    }
                }
        ...
        internal class ContentItem
        {
            ...
            public bool MultiModalFormat { get; internal set; } = false;
        ...
        internal class ContentDataConverter : JsonConverter
        {
            ...
            {
                // If there is only one item and it is not a multi-modal format, just serialize the text
                var item = value as IList<ContentItem>;
                if (item != null && item.Count == 1 && !item[0].MultiModalFormat)
                {
                    serializer.Serialize(writer, item[0].Text);
                }
                else
                {
                    // Otherwise, serialize the whole list
